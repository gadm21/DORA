...
def get_heterogeneous_model(client_id, dataset_shape, n_classes) : 
    ...
    if len(dataset_shape) == 3 : 
        ...
        if model_id == 0 : 
            model = HAR_CV_Net(input_shape = [20, 20], f1 = 2, f2 = 8, f3 = 25, n_classes = 5)
        elif model_id == 1 :
            model = HAR_CV_Net(input_shape = [20, 20], f1 = 4, f2 = 12, f3 = 25, n_classes = 5)
        else :
...
class FLClient(nn.Module) : 
    ...
    def recv_params(self, metadata, compressed = False) : 
        ...
        if self.params['aggregate'] == 'grads' : 
            self.model.load_state_dict(copy.deepcopy(metadata)) 
        else : 
            self.public_set = (self.public_set[0], metadata )

    def communicate_meta(self, alpha, beta, augment) : 
        ...
        self.lambdaa = np.random.beta(self.alpha, self.alpha)
    ...
    def save_assets(self) : 
        ...
        local_log_path = os.path.join(self.client_path, "central_log.csv")
...
class Aggregator () : 
    ...
    def aggregate(self, clients, idxs_users, weights) :
        if self.aggregation_method == "grads" : 
    ...
    def aggregate_soft_labels(clients, idxs_users, weights = None, compress = False) : 
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        all_soft_labels = [clients[idx].get_soft_labels(normalize = False, compress = compress) for idx in idxs_users]
...
class FLServer(nn.Module):
    ...
    def broadcast_meta(self, beta, alpha):
        for client in self.clients:
            client.communicate_meta(beta, alpha, augment = self.augment)
    ...
    def global_update(self):
        ...
        if self.params['aggregate'] == 'soft_labels':
            ...
            if self.params['augment'] : 
                self.broadcast_meta(beta = np.random.randint(1, 1000), alpha = np.random.randint(1, 1_000_000))
            ...
            self.broadcast_meta(beta = np.random.randint(0, 1000), alpha = np.random.randint(1, 1_000_000))
            if self.params['weighting'] == 'uniform':
                weights = [1 for idx in idxs_users]
            else : weights = [self.clients[idx].get_test_acc() for idx in idxs_users]
            ...
            self.broadcast(global_soft_labels)
            self.clients[idx].digest()
        elif self.params['aggregate'] == 'grads':
            if self.params['weighting'] == 'uniform':
                weights = [1 for idx in idxs_users]
            else : weights = [self.clients[idx].get_test_acc() for idx in idxs_users]
            
            all_grads, global_grads = Aggregator.aggregate_weights(self.clients, idxs_users, weights = weights) 
            self.broadcast(global_grads)

        avg_acc = np.mean([self.clients[idx].get_test_acc() for idx in idxs_users])
...
class Experiment(): 
    ...
    def __init__(self, params) : 
        ...
        self.central_accs = pd.read_csv(os.path.join(self.dir, 'client_0', 'central_log.csv'))['local_acc'].values
    ...
    def get_central_acc(self, last = True) :
        if last : return self.central_accs[-1]
        else : return self.central_accs
...
#     central_acc_dict = { dataset : [] for dataset in datasets}
...
#             central_acc = exp.get_central_acc(last = True)
#             central_acc_dict[dataset].append(central_acc)
...
#     for d in datasets : 
#         print(d) 
#         print(central_acc_dict[d])
#     central_row = [np.mean(central_acc_dict[d]) for d in datasets]
#     df.loc[len(df)] = ['central', 'central', *central_row]
