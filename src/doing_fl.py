...
for dataset, C, aggregation_method, ht, aug, weighting, n_pub_sets, dp in product(datasets, Cs, aggregation_methods, HT, Augment, Weighting, n_pub_setss, Private):
    ...
    exp_path = f"../thesis_results/tightDP{dp}/N_pub{n_pub_sets}/{dataset}/Agg{fl_params['aggregate']}_C{fl_params['C']}_HT{fl_params['hyperparameter_tuning']}_Aug{fl_params['augment']}_W{fl_params['weighting']}"
